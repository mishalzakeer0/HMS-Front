...
const PatientSignUp = () => {
// const form = useForm();
const { register, handleSubmit, formState: { errors }, control } = useForm({mode: 'onChange', defaultValues: {registration_date: new Date()}}) ;
...
//   const [date, setDate] = useState(new Date())
  ...
  async function SignUp(data) {
    try {
      const response = await axios.post('http://localhost:3001/patient/signUp', data);
    ...
    <Row>
      ...
      <Col md className="bg-info">
        ...
        <form className="p-4" onSubmit={handleSubmit(SignUp)}>
          ...
          />
          <p className="text-danger fs-6 fw-light">{errors.address?.message}</p>
          {/* City is here  */}
          ...
          {errors.registration_date && (
            <p className="text-danger fs-6 fw-light">{errors.registration_date.message}</p>
          )}
          ...
          <button type="submit" className="btn btn-primary" id="sign-up-btn">
            Submit
          </button>
        </form>
